...
const OrderList = () => {
  ...
  const exportToExcel = () => {
    // Tạo sheet với các trường cụ thể
    const data = order.map((item) => {
      item.statusText = "Đợi xác nhận";
      if (item.status === "rejected") {
        item.statusText = "Đã hủy";
      }
      if (item.status === "approved") {
        item.statusText = "Đang xem xét";
      }
      if (item.status === "final") {
        item.statusText = "Đã xác nhận - Đã thanh toán";
      }
      return item;
    });
    const worksheetData = [
    ...
    const worksheet = XLSX.utils.aoa_to_sheet(worksheetData); // Chuyển array thành sheet
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Booking"); // Thêm sheet vào workbook
  ...
  const handleUpdateOrder = async (values) => {
    ...
    try {
      ...
      await axiosClient
        .put("/booking/" + id + "/update-status/", categoryList)
        .then((response) => {
          if (response === undefined) {
            notification["error"]({
              message: `Thông báo`,
  ...
  const handlePrintInvoice = (order) => {
    ...
    <div>
      <img src="/images/logo213.svg" alt="company-logo" height="100" width="100">
      ...
      <p class="text-gray-500 text-md mt-1">
        VAT: 8657671212
                        ...
                        <div class="invoice-details">
                            <p><span class="font-semibold">Tên sân:</span> ${
                              order.court_name
                            }</p>
                            <p><span class="font-semibold">Ngày đặt:</span> ${formattedDate}</p>
                            <p><span class="font-semibold">Giờ bắt đầu:</span> ${
                              order.start_time
                            }</p>
                            <p><span class="font-semibold">Giờ kết thúc:</span> ${
                              order.end_time
                            }</p>
                        </div>
    ...
    html2pdf()
      .set({
        html2canvas: {
          allowTaint: true, // Cho phép tải tài nguyên bên ngoài
          useCORS: true, // Kích hoạt CORS
        },
      })
      .from(htmlContent)
      .save(`invoice_${order.id}.pdf`);
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (slugs) => (
        <span>
          {slugs === "rejected" ? (
            <Tag style={{ width: 170, textAlign: "center" }} color="red">
              Đã hủy
            </Tag>
          ) : slugs === "approved" ? (
            <Tag
            ...
            <Tag color="green" style={{ width: 170, textAlign: "center" }}>
              Đã xác nhận - Đã thanh toán
            </Tag>
          ) : (
            <Tag color="blue" style={{ width: 170, textAlign: "center" }}>
              Đợi xác nhận
            </Tag>
          )}
        </span>
          ...
          <div style={{ marginTop: 30 }}>
            <div className="flex justify-end mr-4">
              <Button
                ...
                color="default"
                variant="solid"
                className="mb-4 ml-auto text-right"
