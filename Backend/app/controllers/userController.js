...
const userController = {
  getAllClient: async (req, res) => {
    try {
      const query = `SELECT * FROM users WHERE role= 'isClient'`;
  ...
  createUser: async (req, res) => {
    try {
      ...
      const [checkPhoneExist] = await db.execute(
      ...
      res
        .status(200)
        .json({ id: userId, email, phone, username, role, status });
    } catch (err) {
      res.status(500).json(err);
    }
  },
  ...
  updateUser: async (req, res) => {
    try {
      const userId = req.params.id;
      const { username, email, password, role, phone, status, image_qr } =
        req.body;
      ...
      const updateQuery =
        "UPDATE users SET username = ?, email = ?, password = ?, role = ?, phone = ?, status = ?, image_qr = ? WHERE id = ?";
  ...
  updateProfile: async (req, res) => {
    try {
      const userId = req.params.id;
      const { username, email, phone, status, image, role, image_qr } =
        req.body;
  ...
  changePassword: async (req, res) => {
    try {
      ...
      const updatedValues = [hashedNewPassword || null, userId];
